...
describe('MongoDB Connection Builder parsing', ()=>{
  ...
  test('returns a configuration object with an array of two options', () => {
    ...
    expect(config.options![0].key).toEqual('authDb')
    expect(config.options![0].value).toEqual('admin')
    expect(config.options![1].key).toEqual('connectTimeoutMS')
    expect(config.options![1].value).toEqual(300000)
  })
  ...
  test('returns a configuration object with multiple options set', () => {
    ...
    expect(config.options![0].key).toEqual('authSource')
    expect(config.options![0].value).toEqual('admin')
    expect(config.options![1].key).toEqual('replicaSet')
    expect(config.options![1].value).toEqual('VentureChurchCluster1-shard-0')
    expect(config.options![2].key).toEqual('readPreference')
    expect(config.options![2].value).toEqual('primary')
    expect(config.options![3].key).toEqual('appname')
    expect(config.options![3].value).toEqual('MongoDB Compass')
    expect(config.options![4].key).toEqual('ssl')
    expect(config.options![4].value).toEqual(true)
  })
})
